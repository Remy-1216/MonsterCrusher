...
void Knight::StageProcess()
{
	...
	if (m_stageClear == 1)
	{
		if (m_pos.x < kStage1playerPosX + kAbj)
		{
			m_pos.x = kStage1playerPosX + kAbj;
		}
	}
	if (m_stageClear == 2)
	{
		if (m_pos.x < kStage2playerPosX + kAbj)
		{
			m_pos.x = kStage2playerPosX + kAbj;
		}
	}
	if (m_stageClear == 3)
	{
		if (m_pos.x < kStage3playerPosX + kAbj)
		{
			m_pos.x = kStage3playerPosX + kAbj;
		}
	}
...
void Knight::DirectionFacing()
{
	...
	//DrawFormatString(100, 100, 0xffffff, "%1d", rotationDegrees);
...
//のけ反り
void Knight::LeaningBack(int direction)
{
	//のけ反り
...
//敵と当たった時の処理
void Knight::HitBody(int direction)
{
	LeaningBack(direction);
...
void Knight::ConditionCleared(bool clear)
{
	...
	else
	{
		...
		if (m_stageClear == 3)
		{
			if (m_pos.x > kStage4playerPosX - kAbj)
			{
				m_pos.x = kStage4playerPosX- kAbj;
			}
		}
	}
}
